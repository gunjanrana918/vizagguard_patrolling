...
class _ScanningreportState extends State<Scanningreport> {
  ...
  Sendreprtdata() async{
    ...
    var request = http.MultipartRequest('POST', Uri.parse('http://103.25.130.254/VizagGaurdPetroling/Api/Scanreport'));
    request.fields.addAll({
      ...
      'reportlocation':  widget.scandata!.qrcodeTable.locationName,
       'reportcheckpoint': checkpointdata,
       'Uncheckpoint':uncheckpointdata,
  ...
  void _onCategorySelected(bool selected, category_id,) {
    ...
    if (selected == true) {
      setState(() {
        ...
        var mydata = _selecteCategorys.toString();
        String realdata = mydata.replaceAll('[', '');
         checkpointdata = realdata.replaceAll(']', '');
        print("ABCD");
        ...
        for ( i = 0; i < widget.scandata!.qrcodeTable.checkPoitTable.length; i++) {
          if(!_selecteCategorys.contains(widget.scandata!.qrcodeTable.checkPoitTable[i].checkPointName))
          {
            ...
            var uncheckdata= Unselectitem.toString();
            String unrealdata = uncheckdata.replaceAll('[', '');
            uncheckpointdata=unrealdata.replaceAll(']', '');
          }
        }
        print("data");
        print(uncheckpointdata);
